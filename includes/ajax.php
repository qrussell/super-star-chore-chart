...
function sscc_ajax_sscc_archive_week() {
    ...
    $wpdb->insert( "{$wpdb->prefix}sscc_archives", [ 'family_id' => $fid, 'week_of' => $week_of, 'archive_json' => wp_json_encode( $kids ), 'archived_by' => $uid ], [ '%d', '%s', '%s', '%d' ] );
    ...
    if ( $useDefaults ) {
        ...
        $new_kids  = [];
        foreach ( $kid_names as $i => $kname ) { $new_kids[] = [ 'id' => $kids[$i]['id'] ?? ( 'kid_' . $i ), 'name' => $kname, 'categories' => sscc_clone_defaults( $defs ) ]; }
    } else {
...
function sscc_clone_defaults( $defs ) { foreach ( $defs as &$cat ) { foreach ( $cat['tasks'] as &$task ) { $task['checks'] = sscc_empty_checks(); } } return $defs; }
function sscc_clear_all_checks( $kids ) { foreach ( $kids as &$kid ) { foreach ( $kid['categories'] as &$cat ) { foreach ( $cat['tasks'] as &$task ) { $task['checks'] = sscc_empty_checks(); } } } return $kids; }
